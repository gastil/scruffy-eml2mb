...
<!-- 
            ...
            <xsl:choose>
              ...
              attribute name equals metabase "variable"    
                  -->
                            ...
                            <xsl:when test="measurementScale/*/*/bounds">
                                <xsl:for-each select="measurementScale/*/*/bounds">
                                    <!-- 
                                        ...
                                        apply the serialization template -->
                        ...
                        <xsl:value-of select="$delimiter"/>
                        <xsl:choose>
                            <xsl:when test="measurementScale/*/*/bounds/minimum/@exclusive">
                                <xsl:apply-templates select="."></xsl:apply-templates>
                            </xsl:when>
                        ...
                        <xsl:value-of select="$delimiter"/>
                        <xsl:choose>
                            <xsl:when test="measurementScale/*/*/bounds/maximum/@exclusive">
                                <xsl:apply-templates select="."></xsl:apply-templates>
                            </xsl:when>
    ...
    <!-- "@*[name(.)='exclusive']|node()" instead of just dot  or match="bounds|minimum/@exclusive" 
    <xsl:attribute name="exclusive"><xsl:value-of select="@exclusive"/></xsl:attribute> -->
    ...
    <xsl:template match="@exclusive" mode="serialize">
        <xsl:text> </xsl:text>
    ...
    <!-- the below stripped all xml and just dumped content of nodes and attributes 
    <xsl:template name="serialize-bounds-exclusive" match="bounds">
        <xsl:variable name="node_as_string" select="string(.)" />                    
        <xsl:copy-of select="$node_as_string"/>
    </xsl:template> -->
